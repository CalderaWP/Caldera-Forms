...
<script type="text/html" id="conditions-tmpl">
	...
	<div class="caldera-editor-processors-panel" style="margin-bottom: 32px;">
		<ul class="active-processors-list">
			{{#each conditions}}
				<li class="caldera-processor-nav {{#is @root/_open_condition value=id}}active{{/is}} caldera-forms-condition-group condition-point-{{id}}" style="">
					...
					{{#if name}}			
						...
						{{/is}}
						<a data-open-group="{{id}}" style="cursor:pointer;"><span id="condition-group-{{id}}">{{name}}</span> <span class="processor-line-number"></span></a>
					{{else}}
	...
	{{#find conditions @root/_open_condition}}
		<div class="caldera-editor-processor-config caldera-forms-condition-edit" style="margin-top: -27px; width:auto;">
			{{#if name}}
					...
					{{#each group}}
						...
						{{#each this}}
							...
							<div class="caldera-condition-line condition-line-{{@key}}">
								...
								<span style="text-align:right;display:inline-block; width:25px; margin-right:6px;">{{#if @first}}
								...
								<select style="max-width:120px;" name="conditions[{{../../id}}][group][{{parent}}][{{@key}}][field]" data-sync="#condition-bound-field-{{@key}}">
									<option></option>
									{{#each @root/fields}}
								...
								<select style="max-width:110px;" name="conditions[{{../../id}}][group][{{parent}}][{{@key}}][compare]">
								...
								<span data-value="" class="caldera-conditional-field-value" style="padding: 0 12px 0; display:inline-block;">
								{{#find @root/fields field}}
									{{#if config/option}}
										<select style="width:165px;" name="conditions[{{../../../../id}}][group][{{../../parent}}][{{@key}}][value]">
											<option></option>
											{{#each config/option}}
									...
									{{else}}
										<input type="text" style="width: 165px;" class="magic-tag-enabled block-input" name="conditions[{{../../../../id}}][group][{{../../parent}}][{{@key}}][value]" value="{{../../value}}" {{#unless ../../field}}placeholder="Select field first" disabled=""{{/unless}}>
									{{/if}}
								{{/find}}
								</span>
