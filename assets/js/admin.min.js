...
function Caldera_Forms_Panel_Loader( attr, $ ){

    var self = this;

    var $panels;
    this.init = function(){
        $panels = $( '.cf-canvas-ajax' );
        if( 0 != $panels.length ){
            loadLoop();
        }
    }
    ...
    this.loadPanel = function( panel ){
        var $panel = $( panel );
        var name = $panel.data( attr );
        $.ajax({
            url: ajaxurl,
            data: {
                action: 'caldera_forms_ajax_canvas',
                form: $panel.data( 'cf-canvas-ajax-form' ),
                panel: name,
                nonce: $panel.data( 'cf-canvas-ajax-nonce' )
            }
        }).always( function( data ){
            if( 'object' == typeof  data.data && 'string' == typeof data.data.html ){
                $panel.replaceWith( data.data.html );
            }else{
                $panel.html( 'poop' );
            }
...
jQuery( document ).ready( function( $){
    var panelLoader = new Caldera_Forms_Panel_Loader( 'cf-canvas-ajax', $ );
    panelLoader.init();
