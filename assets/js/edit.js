...
jQuery(document).ready(function($) {
	...
	// Change Field Type
	$(".caldera-editor-body").on("change", ".caldera-select-field-type", function(
		e
	) {
	...
	$(".caldera-editor-body").on(
		...
		function(e) {
			...
			if (field_id.substr(0, 1) !== "{") {
				...
				options_wrap = field_wrapper.find(
			...
			if (options_wrap.length) {
				var options_rows = options_wrap.find(".toggle_option_row"),
					out =
						'<select name="' +
						name +
						'[value]" class="caldera-processor-value-bind caldera-conditional-value-field" data-field="' +
						field_id +
				...
				options_rows.each(function(k, v) {
					...
					if (target.data("value")) {
						if (
							target.data("value").toString() === value ||
							target.data("value").toString() ===
								$(v)
				...
				out +=
					'<option value="click" ' +
					(target.data("value") === "click" ? 'selected="selected"' : "") +
					">Click</option>";
				out +=
					'<option value="dblclick" ' +
					(target.data("value") === "dblclick" ? 'selected="selected"' : "") +
					">Double Click</option>";
				out += "</select>";
			} else {
				out =
					'<input name="' +
					name +
					'[value]" type="text" class="caldera-conditional-value-field magic-tag-enabled" value="' +
	...
	// conditionals
	$(".caldera-editor-body").on("click", ".remove-conditional-line", function(
		e
	) {
		e.preventDefault();
		var clicked = $(this),
...
jQuery(document).ready(function($) {
	...
	focus_initial_field = function(e) {
		...
		if (field.length) {
			...
			$(".layout-column.column-container")
				.first()
				.trigger("mouseover");
