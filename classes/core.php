...
class Caldera_Forms
{
	...
	function __construct()
	{
		...
		add_filter('shortcode_atts_caldera_form_modal', array('Caldera_Forms_Shortcode_Atts', 'maybe_allow_revision'),
			5, 4);
		...
		add_filter('caldera_forms_autoresponse_mail',
			array('Caldera_Forms_Email_Filters', 'format_autoresponse_message'));
		...
		add_action('caldera_forms_entry_actions', array(Caldera_Forms_Entry_UI::get_instance(), 'get_entry_actions'),
			1);
		...
		add_shortcode(Caldera_Forms_Entry_Shortcode::get_shortcode_name(),
			array('Caldera_Forms_Entry_Shortcode', 'shortcode_callback'));
	...
	static public function run_calculation($value, $field, $form)
	{
		...
		if(phpversion() < '7.2.0'){
			...
			if( $deprecated === false ){ 
				...
				if($result[0] === "done"){ 
					$total = $result['1'];
				} else if($result[0] === "error") {  //Return an error if parser returned an error status
					return new WP_Error($field['ID'] . '-calculation', __('Deprecated operator or symbol in calculation for PHP version >= 7.2 requirements', 'caldera-forms'));
				}
				
			} else { //Return an error with the deprecated symbol or function caught
				$message = sprintf(
					'"%s" %s',
					$deprecated,
					__('has been deprecated in calculations for PHP version >= 7.2', 'caldera-forms')
				);
				return new WP_Error($field['ID'] . '-calculation', $message);
			}	
